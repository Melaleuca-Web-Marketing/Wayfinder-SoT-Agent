...
- **Quality cleanup retry:** set `ENABLE_ANSWER_QUALITY_RETRY=true` (default) to run one guarded cleanup pass when the checker detects corrupted wording. Configure checker model with `QUALITY_CHECK_MODEL` (default `gpt-4.1`), rewrite model with `QUALITY_REWRITE_MODEL` (default `gpt-4.1`), and target model prefixes with `QUALITY_RETRY_MODEL_PREFIXES` (default `gpt-5.4,gpt-5.3-chat`).
